...
function renderResultBox(){ const result = state.latestDrawResult; if(!result){ els.drawResultBox.innerHTML='<div class="empty-state">尚未抽獎</div>'; return; } const rows = Array.isArray(result.results)?result.results:(Array.isArray(result.draw_results)?result.draw_results:[]); const spent = result.points_spent ?? result.total_points_spent ?? null; els.drawResultBox.innerHTML=`<div class="result-summary">${spent!=null?`本次扣除 ${Number(spent)} 點`:'抽獎完成'}</div>` + (rows.length?rows.map((row,idx)=>`<div class="list-item"><div><div class="list-title">第 ${Number(row.draw_index ?? idx+1)} 抽 ・ ${escapeHtml(row.prize_code || '')} ${escapeHtml(row.prize_name || '未中獎')}</div><div class="list-subtitle">票號 ${escapeHtml(row.ticket_no ?? '-')}</div></div></div>`).join(''):'<div class="empty-state">本次沒有回傳獎項資料</div>'); }
