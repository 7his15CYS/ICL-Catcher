...
function renderPrizeList(prizes){
  els.prizeList.innerHTML = prizes.length
    ? prizes.map(p=>`<article class="prize-card"><img class="prize-image" src="${escapeHtml(p.prize_image_url || 'https://placehold.co/240x180?text=Prize')}" alt="${escapeHtml(p.prize_name)}"><div class="prize-body"><div class="prize-code">${escapeHtml(p.prize_code)}</div><h4>${escapeHtml(p.prize_name)}</h4><div class="prize-meta">剩餘 ${Number(p.remaining_quantity||0)} / 共 ${Number(p.total_quantity||0)}</div></div></article>`).join('')
